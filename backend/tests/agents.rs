...
async fn list_agents_rejects_invalid_pagination() {
    ...
    for query in [
        ...
        let (status, _, _) = app
            .request("GET", &format!("/api/agents?{query}"), Some(&admin), None)
            .await;
...
async fn list_agents_filters() {
    ...
    let member = app
        .seed_agent("findme@test.com", "password1", "agent")
        .await;
    app.add_membership(&member, team, "member", true).await;
    let inactive = app
        .seed_agent("sleepy@test.com", "password1", "agent")
        .await;
    ...
    let (_, body, _) = app
        .request(
            "GET",
            "/api/agents?includeInactive=true",
            Some(&admin),
            None,
        )
        .await;
    ...
    let (_, body, _) = app
        .request("GET", "/api/agents?search=findme", Some(&admin), None)
        .await;
    ...
    assert_eq!(body["data"].as_array().unwrap().len(), 1);
    let (_, body, _) = app
        .request("GET", "/api/agents?role=admin", Some(&admin), None)
        .await;
    assert_eq!(body["data"].as_array().unwrap().len(), 1);

    // Unknown status values are ignored, not rejected (CRD 2165).
    let (status, _, _) = app
        .request("GET", "/api/agents?status=zzz", Some(&admin), None)
        .await;
    assert_eq!(status, StatusCode::OK);
}
...
async fn batch_update_validation_and_authorization() {
    ...
    let (status, _, _) = app
        .request(
            ...
            "/api/agents/batch",
            Some(&admin),
            Some(json!({"agentIds": [], "updates": {"isActive": true}})),
        )
        .await;
    ...
    let (status, _, _) = app
        .request(
            ...
            "/api/agents/batch",
            Some(&admin),
            Some(json!({"agentIds": many, "updates": {"isActive": true}})),
        )
        .await;
    assert_eq!(status, StatusCode::BAD_REQUEST);
    let (status, _, _) = app
        .request(
            ...
            "/api/agents/batch",
            Some(&admin),
            Some(json!({"agentIds": ["short"], "updates": {"isActive": true}})),
        )
        .await;
    assert_eq!(status, StatusCode::BAD_REQUEST);
    ...
    let (status, _, _) = app
        .request(
            ...
            "/api/agents/batch",
            Some(&leader),
            Some(json!({"agentIds": ["agent-id-x-1"], "updates": {"isActive": true}})),
        )
        .await;
    assert_eq!(status, StatusCode::FORBIDDEN);
}
...
async fn batch_transfer_errors() {
    ...
    let (status, body, _) = app
        .request(
            ...
            "/api/agents/batch/transfer",
            Some(&admin),
            Some(json!({"agentIds": [agent], "toTeamId": 9999})),
        )
        .await;
...
async fn search_agents_filters_and_orders() {
    ...
    let hit = app
        .seed_agent("needle@test.com", "password1", "agent")
        .await;
    ...
    let (_, body, _) = app
        .request(
            ...
            "/api/agents/search",
            Some(&admin),
            Some(json!({"teamIds": [team], "isActive": true})),
        )
        .await;
    assert_eq!(body["data"].as_array().unwrap().len(), 1);
...
async fn status_statistics_counts_presence_states() {
    ...
    let (status, _, _) = app
        .request(
            "PUT",
            &format!("/api/agents/{online}/status"),
            Some(&token),
    ...
    let (status, body, _) = app
        .request("GET", "/api/agents/status/statistics", Some(&admin), None)
        .await;
    assert_eq!(status, StatusCode::OK);
    ...
    // Ordinary operators forbidden.
    let (status, _, _) = app
        .request("GET", "/api/agents/status/statistics", Some(&token), None)
        .await;
    assert_eq!(status, StatusCode::FORBIDDEN);
}
...
async fn skills_crud_with_scoping() {
    ...
    let (status, body, _) = app
        .request(
            "GET",
            &format!("/api/agents/{agent}/skills"),
            Some(&token),
            None,
        )
        .await;
    ...
    let (status, _, _) = app
        .request("GET", "/api/agents/short/skills", Some(&token), None)
        .await;
    assert_eq!(status, StatusCode::BAD_REQUEST);
    ...
    let (status, _, _) = app
        .request(
            "GET",
            &format!("/api/agents/{other}/skills"),
            Some(&token),
            None,
        )
        .await;
...
async fn add_skill_validation_errors() {
    ...
    for bad in [
        ...
        let (status, _, _) = app
            .request("POST", &path, Some(&token), Some(bad.clone()))
            .await;
    ...
    let (status, _, _) = app
        .request("POST", &path, Some(&token), Some(skill_payload().await))
        .await;
...
async fn update_skill_merges_and_handles_certification() {
    ...
    let (status, _, _) = app
        .request(
            "PUT",
            &format!("/api/agents/{agent}/skills/ghost"),
            Some(&token),
            Some(json!({"level": "expert"})),
        )
        .await;
    assert_eq!(status, StatusCode::INTERNAL_SERVER_ERROR);
}
...
async fn skill_statistics_summary() {
    ...
    let (status, body, _) = app
        .request("GET", &format!("{path}/statistics"), Some(&token), None)
        .await;
    assert_eq!(status, StatusCode::OK);
...
async fn get_status_defaults_to_offline_and_auto_expires() {
    ...
    let (status, body, _) = app
        .request(
            "GET",
            &format!("/api/agents/{agent}/status"),
            Some(&token),
            None,
        )
        .await;
...
async fn update_status_validates_and_records_history() {
    ...
    let (status, body, _) = app
        .request(
            ...
            &path,
            Some(&token),
            Some(json!({"status": "busy", "availableUntil": future, "note": "deep work"})),
        )
        .await;
    assert_eq!(status, StatusCode::OK, "{body}");
    ...
    for bad in [
        ...
        let (status, _, _) = app
            .request("PUT", &path, Some(&token), Some(bad.clone()))
            .await;
        assert_eq!(status, StatusCode::BAD_REQUEST, "{bad}");
    }
...
async fn status_history_newest_first_with_limit() {
    ...
    for s in ["online", "busy", "away"] {
        app.request("PUT", &path, Some(&token), Some(json!({"status": s})))
            .await;
    }
...
async fn get_agent_combines_profile_skills_and_status() {
    ...
    let (status, body, _) = app
        .request("GET", &format!("/api/agents/{agent}"), Some(&token), None)
        .await;
    assert_eq!(status, StatusCode::OK, "{body}");
...
async fn get_agent_error_conditions() {
    ...
    let (status, _, _) = app
        .request(
            "GET",
            "/api/agents/agent-0000-0000-0000-000000000000",
            Some(&admin),
            None,
        )
        .await;
    ...
    // Identifier length out of range -> 400.
    let (status, _, _) = app
        .request("GET", "/api/agents/tiny-id", Some(&admin), None)
        .await;
    assert_eq!(status, StatusCode::BAD_REQUEST);
    ...
    let (status, _, _) = app
        .request("GET", &format!("/api/agents/{a}"), Some(&token), None)
        .await;
    assert_eq!(status, StatusCode::FORBIDDEN);
}
...
async fn update_agent_validation_errors() {
    ...
    for bad in [
        ...
        let (status, _, _) = app
            .request("PUT", &path, Some(&admin), Some(bad.clone()))
            .await;
        assert_eq!(status, StatusCode::BAD_REQUEST, "{bad}");
    }
...
async fn update_agent_privilege_guards() {
    ...
    let (status, _, _) = app
        .request(
            ...
            &format!("/api/agents/{agent}"),
            Some(&token),
            Some(json!({"displayName": "My New Name"})),
        )
        .await;
    assert_eq!(status, StatusCode::OK);
}
...
async fn update_agent_collision_and_team_errors_surface_as_500() {
    ...
    let (status, _, _) = app
        .request(
            ...
            &format!("/api/agents/{agent}"),
            Some(&admin),
            Some(json!({"email": "taken@test.com"})),
        )
        .await;
    assert_eq!(status, StatusCode::INTERNAL_SERVER_ERROR);
