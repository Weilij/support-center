...
async fn seed_conversations(app: &TestApp, team: i64, n: usize, closed: usize) {
    for i in 0..n {
        let customer = app
            .seed_customer("line", &format!("U-an-{team}-{i}"), "C", Some(team))
            .await;
...
#[tokio::test]
async fn analytics_routes_require_analytics_position() {
    let app = spawn_app().await;
    let token = plain_agent(&app).await;
    let (status, _, _) = app
        .request("GET", "/api/analytics/conversations", Some(&token), None)
        .await;
...
async fn conversation_analytics_summary_trends_distribution() {
    ...
    let (status, body, _) = app
        .request(
            "GET",
            "/api/analytics/conversations?timeRange=7d",
            Some(&admin),
            None,
        )
        .await;
...
async fn non_admins_are_scoped_to_their_team() {
    ...
    let (_, body, _) = app
        ...
        .await;
    assert_eq!(
        body["data"]["data"]["summary"]["totalConversations"], 3,
...
async fn message_and_performance_analytics() {
    ...
    for i in 0..4 {
        app.seed_message(&conversation, "customer", &format!("m{i}"), None)
            .await;
    }
    ...
    assert!(
        body["data"]["data"]["summary"]["messagesPerHour"]
            .as_f64()
            .unwrap()
            > 0.0
    );
    assert_eq!(
        body["data"]["data"]["typeDistribution"][0]["category"],
        "text"
    );
    ...
    assert_eq!(
        body["data"]["metadata"]["aggregation"], "hourly",
        "24h default window"
    );
}
...
async fn custom_query_and_export_require_query_permission() {
    ...
    assert_eq!(
        ...
        "plain agents lack the query level"
    );
...
async fn metrics_record_and_bucketed_query() {
    ...
    let (status, _, _) = app
        .request(
            ...
            Some(&admin),
            Some(
                json!({"metric": {"id": "m1", "name": "cpu", "value": "high",
                                   "timestamp": 1, "tags": {}}}),
            ),
        )
        .await;
    ...
    let batch: Vec<_> = (0..4)
        .map(|i| {
            json!({"id": format!("m{i}"), "name": "cpu", "value": (i + 1) as f64 * 10.0,
    ...
    let (status, body, _) = app
        .request(
            "GET",
            &format!(
                "/api/analytics/metrics/cpu?startTime={}&endTime={}",
                base - 1000,
                base + 10_000
            ),
            Some(&admin),
    ...
    let (status, _, _) = app
        .request(
            "GET",
            "/api/analytics/metrics/cpu?startTime=10&endTime=5",
            Some(&admin),
            None,
        )
        .await;
...
async fn comparison_single_multi_and_presets() {
    ...
    let cur_s = (now - chrono::Duration::days(7))
        .format("%Y-%m-%dT%H:%M:%SZ")
        .to_string();
    // End one minute ahead so same-second fractional timestamps stay inside.
    let cur_e = (now + chrono::Duration::minutes(1))
        .format("%Y-%m-%dT%H:%M:%SZ")
        .to_string();
    ...
    let (status, _, _) = app
        .request(
            "GET",
            "/api/analytics/comparison/metric?metric=total_conversations",
            Some(&admin),
            None,
        )
        .await;
    ...
    let (status, _, _) = app
        .request(
            "GET",
            "/api/analytics/comparison/cache/stats",
            Some(&admin),
            None,
        )
        .await;
...
async fn dashboard_config_data_and_widgets() {
    ...
    let (_, body, _) = app
        .request(
            "GET",
            "/api/analytics/dashboard/widget-types",
            Some(&agent),
            None,
        )
        .await;
    ...
    let (_, body, _) = app
        .request(
            "GET",
            "/api/analytics/dashboard/templates?category=operations",
            Some(&agent),
            None,
        )
        .await;
    ...
    let (status, body, _) = app
        .request(
            "POST",
            "/api/analytics/dashboard/config/main",
            Some(&admin),
    ...
    assert_eq!(
        body["data"]["permissions"]["owner"],
        "admin@test.dev"
            .split('@')
            .next()
            .map(|_| body["data"]["permissions"]["owner"].as_str().unwrap())
            .unwrap()
    );
    ...
    let (status, body, _) = app
        .request(
            "GET",
            "/api/analytics/dashboard/data/main",
            Some(&admin),
            None,
        )
        .await;
    ...
    let (status, _, _) = app
        .request(
            "GET",
            "/api/analytics/dashboard/data/main?timeRange=not-json",
            Some(&admin),
            None,
        )
        .await;
    ...
    let (status, _, _) = app
        .request(
            "GET",
            "/api/analytics/dashboard/widget/w1/data?dashboardId=main",
            Some(&admin),
            None,
        )
        .await;
    ...
    let (status, _, _) = app
        .request(
            "GET",
            "/api/analytics/dashboard/widget/ghost/data?dashboardId=main",
            Some(&admin),
            None,
        )
        .await;
    ...
    let (status, _, _) = app
        .request(
            "POST",
            "/api/analytics/dashboard/widget",
            Some(&agent),
    ...
    assert_eq!(
        status,
        StatusCode::FORBIDDEN,
        "plain agent lacks team-level role"
    );
    let (status, body, _) = app
        .request(
            "POST",
            "/api/analytics/dashboard/widget/w1/clone",
            Some(&admin),
    ...
    let (status, body, _) = app
        .request(
            "POST",
            "/api/analytics/dashboard/layout/optimize",
            Some(&admin),
    ...
    assert_eq!(
        widgets[1]["position"]["y"], 2,
        "second widget wrapped to the next row"
    );
}
...
async fn realtime_dashboard_control() {
    ...
    app.request(
        "POST",
        "/api/analytics/dashboard/config/rt",
        Some(&admin),
    ...
    let (status, _, _) = app
        .request(
            "POST",
            "/api/analytics/realtime/broadcast",
            Some(&admin),
    ...
    let (status, body, _) = app
        .request(
            "POST",
            "/api/analytics/realtime/trigger-update/rt/w1",
            Some(&admin),
            None,
        )
        .await;
    ...
    let (status, _, _) = app
        .request(
            "POST",
            "/api/analytics/realtime/trigger-update/rt/ghost",
            Some(&admin),
            None,
        )
        .await;
    ...
    let (status, body, _) = app
        .request(
            "POST",
            "/api/analytics/realtime/trigger-update/rt",
            Some(&admin),
            None,
        )
        .await;
    ...
    let (status, _, _) = app
        .request(
            "POST",
            "/api/analytics/realtime/cleanup",
            Some(&admin),
            None,
        )
        .await;
...
async fn security_dashboard_metrics_events_summary() {
    ...
    // Public health.
    let (status, body, _) = app
        .request("GET", "/api/security/dashboard/health", None, None)
        .await;
    assert_eq!(status, StatusCode::OK);
    ...
    let (status, body, _) = app
        .request(
            "GET",
            "/api/security/dashboard/metrics?timeRange=24h",
            Some(&admin),
            None,
        )
        .await;
    ...
    assert_eq!(
        m["corsMonitoring"]["topRejectedOrigins"][0]["origin"],
    ...
    let (status, body, _) = app
        .request(
            "GET",
            "/api/security/dashboard/events/recent?limit=2",
            Some(&admin),
            None,
        )
        .await;
    ...
    let (status, _, _) = app
        .request(
            "GET",
            "/api/security/dashboard/events/recent?limit=0",
            Some(&admin),
            None,
        )
        .await;
