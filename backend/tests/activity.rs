...
async fn setup() -> Ctx {
    ...
    let admin_id = app
        .seed_agent("admin@example.com", "Password1!", "admin")
        .await;
    let agent_id = app
        .seed_agent("agent@example.com", "Password1!", "agent")
        .await;
...
async fn list_newest_first_with_pagination() {
    ...
    let (status, body, _) = ctx
        ...
        .request(
            "GET",
            "/api/activities/?action=order%20probe",
    ...
    let rids: Vec<&str> = items
        .iter()
        .map(|i| i["resourceId"].as_str().unwrap())
        .collect();
...
async fn list_rejects_out_of_range_page_and_limit() {
    ...
    for (path, field) in [
        ...
        let (status, body, _) = ctx
            .app
            .request("GET", path, Some(&ctx.admin_token), None)
            .await;
...
async fn list_rejects_bad_dates() {
    ...
    let (status, body, _) = ctx
        ...
        .request(
            "GET",
            "/api/activities?startDate=not-a-date",
            Some(&ctx.admin_token),
            None,
        )
        .await;
...
async fn list_scopes_non_admin_to_own_entries() {
    let ctx = setup().await;
    ctx.app
        .seed_activity(&ctx.admin_id, "scope probe", "tag", Some("1"), None, None)
        .await;
    ctx.app
        .seed_activity(&ctx.agent_id, "scope probe", "tag", Some("2"), None, None)
        .await;
    ...
    let (status, body, _) = ctx
        .app
        .request("GET", &path, Some(&ctx.agent_token), None)
        .await;
    assert_eq!(status, StatusCode::OK);
    let items = body["data"]["items"].as_array().unwrap();
...
async fn list_admin_filters_by_actor_action_resource_and_window() {
    ...
    let path = format!(
        "/api/activities?userId={}&action=filter%20probe",
        ctx.agent_id
    );
    let (_, body, _) = ctx
        .app
    ...
    assert_eq!(
        body["data"]["total"], 2,
        "window excludes the 10-day-old entry"
    );
}
...
async fn get_activity_visible_to_admin_and_actor() {
    ...
    for token in [&ctx.admin_token, &ctx.agent_token] {
        let (status, body, _) = ctx
            .app
            .request("GET", &format!("/api/activities/{id}"), Some(token), None)
            .await;
        assert_eq!(status, StatusCode::OK, "{body}");
...
async fn get_activity_forbidden_for_non_actor() {
    ...
    let id = ctx
        .app
        .seed_activity(&ctx.admin_id, "secret probe", "tag", None, None, None)
        .await;
    let (status, body, _) = ctx
        .app
        .request(
...
async fn get_activity_not_found_and_invalid_id() {
    ...
    let (status, body, _) = ctx
        .app
        .request("GET", "/api/activities/abc", Some(&ctx.admin_token), None)
        .await;
...
async fn user_stats_aggregate_own_actions() {
    let ctx = setup().await;
    ctx.app
        .seed_activity(&ctx.agent_id, "stat probe a", "tag", None, None, None)
        .await;
    ctx.app
        .seed_activity(&ctx.agent_id, "stat probe a", "tag", None, None, None)
        .await;
    ctx.app
        .seed_activity(&ctx.agent_id, "stat probe b", "tag", None, None, None)
        .await;
    ...
    assert!(!body["data"]["recentActivities"]
        .as_array()
        .unwrap()
        .is_empty());
}
...
async fn cleanup_purges_entries_older_than_retention() {
    ...
    let fresh = ctx
        .app
        .seed_activity(&ctx.agent_id, "fresh probe", "tag", None, None, None)
        .await;
    ...
    let (status, body, _) = ctx
        ...
        .request(
            "POST",
            "/api/activities/cleanup?days=90",
            Some(&ctx.admin_token),
            None,
        )
        .await;
    ...
    assert!(
        common::TestApp::request(
            &ctx.app,
            "GET",
            &format!("/api/activities/{old}"),
            Some(&ctx.admin_token),
            None
        )
        .await
...
async fn cleanup_admin_only_and_retention_bounds() {
    ...
    for path in [
        ...
        let (status, body, _) = ctx
            .app
            .request("POST", path, Some(&ctx.admin_token), None)
            .await;
        assert_eq!(status, StatusCode::UNPROCESSABLE_ENTITY, "{path}: {body}");
...
async fn overview_returns_window_aggregates() {
    let ctx = setup().await;
    ctx.app
        .seed_activity(&ctx.agent_id, "overview probe", "tag", None, None, None)
        .await;
    let (status, body, _) = ctx
        .app
        .request(
...
async fn resource_stats_carry_share_and_label() {
    let ctx = setup().await;
    ctx.app
        .seed_activity(&ctx.agent_id, "res probe", "tag", None, None, None)
        .await;
    let (status, body, _) = ctx
        .app
        .request(
...
async fn role_stats_carry_share_and_label() {
    let ctx = setup().await;
    ctx.app
        .seed_activity(&ctx.agent_id, "role probe", "tag", None, None, None)
        .await;
    let (status, body, _) = ctx
        .app
        .request(
...
async fn trends_report_per_day_action_breakdown() {
    ...
    let with_probe: Vec<&Value> = series
        .iter()
        .filter(|d| d["actions"]["trend probe"].is_number())
        .collect();
...
async fn heatmap_buckets_classify_intensity() {
    let ctx = setup().await;
    ctx.app
        .seed_activity(&ctx.agent_id, "heat probe", "tag", None, None, None)
        .await;
    let (status, body, _) = ctx
        .app
        .request(
...
async fn metrics_summarize_load_and_leaders() {
    let ctx = setup().await;
    ctx.app
        .seed_activity(&ctx.agent_id, "metric probe", "tag", None, None, None)
        .await;
    let (status, body, _) = ctx
        .app
        .request(
...
async fn restore_validates_id_and_existence_before_auth() {
    ...
    let (status, body, _) = ctx
        .app
        .request("POST", "/api/activities/abc/restore", None, None)
        .await;
    ...
    let (status, body, _) = ctx
        .app
        .request("POST", "/api/activities/999999/restore", None, None)
        .await;
    assert_eq!(status, StatusCode::NOT_FOUND);
    assert_eq!(body["error"], "Activity not found");
}
...
async fn restore_rejects_non_reversible_entries() {
    ...
    let id = ctx
        ...
        .seed_activity(
            ...
            "tag",
            Some("1"),
            Some(json!({"reversible": false})),
            None,
        )
        .await;
    let (status, body, _) = ctx
        .app
        .request("POST", &format!("/api/activities/{id}/restore"), None, None)
        .await;
    assert_eq!(status, StatusCode::UNPROCESSABLE_ENTITY);
...
async fn restore_original_actor_allowed_others_forbidden() {
    ...
    let other_id = ctx
        .app
        .seed_agent("other@example.com", "Password1!", "agent")
        .await;
...
async fn restore_window_expiry() {
    ...
    let (status, body, _) = ctx
        ...
        .request(
            "POST",
            &format!("/api/activities/{stale}/restore"),
            Some(&ctx.admin_token),
            None,
        )
        .await;
    ...
    let (status, body, _) = ctx
        ...
        .request(
            "POST",
            &format!("/api/activities/{expired}/restore"),
            Some(&ctx.admin_token),
            None,
        )
        .await;
...
async fn restore_missing_resource_id_rejected() {
    ...
    let id = ctx
        ...
        .seed_activity(
            ...
            "tag",
            None,
            Some(json!({"reversible": true, "old": {"name": "a"}, "new": {"name": "b"}})),
            None,
        )
        .await;
    let (status, body, _) = ctx
...
async fn restore_conversation_transfer_returns_prior_team() {
    ...
    let customer = ctx
        .app
        .seed_customer("line", "u1", "Customer", Some(team_a))
        .await;
    let conv = ctx
        .app
        .seed_conversation(customer, Some(team_b), "active")
        .await;
    let id = ctx
        .app
        .seed_activity(
...
async fn restore_conversation_transfer_detects_drift() {
    ...
    let customer = ctx
        .app
        .seed_customer("line", "u2", "Customer", Some(team_a))
        .await;
    // The conversation drifted to a third team after the recorded transfer.
    let conv = ctx
        .app
        .seed_conversation(customer, Some(team_c), "active")
        .await;
    let id = ctx
        .app
        .seed_activity(
...
async fn restore_tag_delete_undeletes_record() {
    ...
    let tag = ctx
        .app
        .seed_tag_full("gone-tag", &ctx.agent_id, None, false)
        .await;
