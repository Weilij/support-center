...
async fn process_job(state: &Arc<AppState>, job: Job) {
    ...
    let kind = job.body.get("type").and_then(Value::as_str).unwrap_or("outbound_message");
    ...
    match result {
        ...
        Err(error) => {
            let category = super::categorize(&error);
            ...
            if retry {
                ...
                let next = Job { body: job.body, attempt: job.attempt + 1 };
                tokio::spawn(async move {
...
async fn process_outbound(state: &Arc<AppState>, body: &Value) -> Result<(), String> {
    let message_id = body.get("messageId").and_then(Value::as_str).unwrap_or_default();
    let conversation_id = body.get("conversationId").and_then(Value::as_str).unwrap_or_default();
    let recipient = body.get("recipientId").and_then(Value::as_str).unwrap_or_default();
...
async fn process_media(state: &Arc<AppState>, body: &Value) -> Result<(), String> {
    let message_id = body.get("messageId").and_then(Value::as_str).unwrap_or_default();
    let conversation_id = body.get("conversationId").and_then(Value::as_str).unwrap_or_default();
    let platform_message_id =
        body.get("platformMessageId").and_then(Value::as_str).unwrap_or_default();
    let media_type = body.get("mediaType").and_then(Value::as_str).unwrap_or("file");
    ...
    // 1. Fetch + store the media (CRD 5176). The platform content API is
    // stubbed: media already mirrored into the store (e.g. by the line-proxy
    // self-heal) is "fetched"; otherwise the download fails and the job
    // retries. TODO(channels): real LINE content API download.
    let Some(bytes) =
        crate::domain::files::store::get_object(&state.config.upload_dir, &key).await
    else {
        return Err("network: media download yielded no attachment".to_string());
    };
    ...
    match team_id {
        Some(t) => state.realtime.to_team(t, "message_updated", payload.clone()),
        None => state.realtime.global("message_updated", payload.clone()),
    };
    // 4. Best-effort per-conversation notification (CRD 5179).
    state.realtime.to_conversation(conversation_id, "message_updated", payload);
    Ok(())
}
