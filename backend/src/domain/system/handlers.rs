    ...
    if let Some(general) = body.get("general") {
        if let Some(name) = general.get("systemName").and_then(Value::as_str) {
            if name.is_empty() || name.chars().count() > 100 {
                return Err(AppError::BadRequest("systemName must be 1-100 characters".into()));
            }
        }
        if let Some(email) = general.get("contactEmail").and_then(Value::as_str) {
            if !email.contains('@') {
                return Err(AppError::BadRequest("contactEmail must be a valid email".into()));
            }
        }
        if let Some(lang) = general.get("language").and_then(Value::as_str) {
            if !["en", "zh-TW", "zh-CN", "ja"].contains(&lang) {
                return Err(AppError::BadRequest("language must be one of en/zh-TW/zh-CN/ja".into()));
            }
        }
    }
...
pub async fn update_settings(
    ...
    let ua = headers.get("user-agent").and_then(|v| v.to_str().ok()).map(str::to_string);
    crate::domain::auth::store::log_activity(
        &state.db, &user.id, &user.display_name, &user.role,
        "settings_update", "system", None,
        Some(json!({
...
pub async fn metrics(
    ...
    let conversations: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM conversations WHERE deleted_at IS NULL")
            .fetch_one(&state.db).await.unwrap_or(0);
    let messages: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM messages WHERE deleted_at IS NULL")
        .fetch_one(&state.db).await.unwrap_or(0);
    Ok(envelope::ok(json!({
        "activeAgents": active,
...
pub async fn test_integration(
    ...
    let result = match platform.as_str() {
        "line" => {
            let complete = body.channel_id.as_deref().map(|v| !v.is_empty()).unwrap_or(false)
                && body.channel_secret.as_deref().map(|v| !v.is_empty()).unwrap_or(false)
                && body.access_token.as_deref().map(|v| !v.is_empty()).unwrap_or(false);
            if complete {
                // TODO(channels): live LINE token-verify call (CRD 5342).
                json!({"status": "success", "message": "LINE 整合測試通過",
                       "details": {"channelId": body.channel_id, "testedAt": now_iso()}})
            } else {
                json!({"status": "error", "message": "請先完成 LINE 設定"})
            }
        }
        "facebook" => {
            let complete = body.app_id.as_deref().map(|v| !v.is_empty()).unwrap_or(false)
                && body.app_secret.as_deref().map(|v| !v.is_empty()).unwrap_or(false)
                && body.page_id.as_deref().map(|v| !v.is_empty()).unwrap_or(false)
                && body.page_token.as_deref().map(|v| !v.is_empty()).unwrap_or(false);
            if complete {
                json!({"status": "success", "message": "Facebook 整合測試通過",
                       "details": {"pageId": body.page_id, "testedAt": now_iso()}})
            } else {
...
pub async fn config_check(
    ...
    let code = if satisfied { StatusCode::OK } else { StatusCode::SERVICE_UNAVAILABLE };
...
pub async fn health_component(
    ...
    let verdict = match component.as_str() {
        "database" => {
            if db_ok(&state).await { "healthy" } else { "critical" }
        }
        "cache" => "healthy",
        _ => "unknown",
    };
    (
        ...
        Json(json!({"success": verdict == "healthy" || verdict == "warning",
...
pub async fn health_ready(State(state): State<Arc<AppState>>) -> Response {
    ...
    let code = if verdict == "critical" { StatusCode::SERVICE_UNAVAILABLE } else { StatusCode::OK };
    (code, Json(json!({"ready": verdict != "critical", "timestamp": now_iso()}))).into_response()
}

pub async fn health_live() -> Response {
    (StatusCode::OK, Json(json!({"alive": true, "timestamp": now_iso()}))).into_response()
}
...
pub async fn feedback_stats(
    ...
    for star in 1..=5 {
        let count = rows.iter().find(|(r, _)| *r == star).map(|(_, c)| *c).unwrap_or(0);
...
pub async fn feedback_for_conversation(
    ...
    type FbRow = (String, i64, Option<String>, Option<String>, Option<String>, String);
    ...
    let items: Vec<Value> = rows
        .iter()
        .map(|(id, rating, comment, customer, agent, created)| json!({
            "id": id, "rating": rating, "comment": comment,
            "customerName": customer, "agentName": agent, "createdAt": created,
        }))
        .collect();
    Ok(envelope::ok(json!({ "feedback": items, "count": items.len() })))
}
...
pub async fn feedback_list(
    ...
    type FbRow = (String, String, i64, Option<String>, Option<String>, Option<String>, String);
    let rows: Vec<FbRow> = sqlx::query_as(
    ...
    let items: Vec<Value> = rows
        .iter()
        .map(|(id, conv, rating, comment, customer, agent, created)| json!({
