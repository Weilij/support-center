...
pub fn routes(state: Arc<AppState>) -> Router<Arc<AppState>> {
    ...
    let authed = Router::new()
        ...
        .route(
            "/api/analytics/comparison/preset/conversation",
            get(comparison::preset_conversation),
        )
        .route(
            "/api/analytics/comparison/preset/message",
            get(comparison::preset_message),
        )
        .route(
            "/api/analytics/comparison/preset/user-activity",
            get(comparison::preset_user_activity),
        )
        .route(
            "/api/analytics/comparison/cache/stats",
            get(comparison::cache_stats),
        )
        // Dashboards.
        .route("/api/analytics/dashboard/health", get(dashboard::health))
        .route(
            "/api/analytics/dashboard/widget-types",
            get(dashboard::widget_types),
        )
        .route(
            "/api/analytics/dashboard/templates",
            get(dashboard::list_dashboard_templates),
        )
        .route(
            "/api/analytics/dashboard/widget-templates",
            get(dashboard::list_widget_templates),
        )
        .route(
            "/api/analytics/dashboard/layout/optimize",
            post(dashboard::optimize_layout),
        )
        .route(
            "/api/analytics/dashboard/config",
            get(dashboard::get_config)
                .post(dashboard::save_config)
                .put(dashboard::save_config),
        )
        .route(
        ...
        .route(
            "/api/analytics/dashboard/data/{dashboardId}",
            get(dashboard::dashboard_data),
        )
        .route(
            "/api/analytics/dashboard/widget",
            post(dashboard::create_widget),
        )
        .route(
            "/api/analytics/dashboard/widget/{widgetId}",
            put(dashboard::update_widget),
        )
        .route(
            "/api/analytics/dashboard/widget/{widgetId}/data",
            get(dashboard::widget_data),
        )
        .route(
            "/api/analytics/dashboard/widget/{widgetId}/clone",
            post(dashboard::clone_widget),
        )
        .route(
        ...
        .route(
            "/api/analytics/realtime/status",
            get(dashboard::realtime_status),
        )
        .route(
            "/api/analytics/realtime/health",
            get(dashboard::realtime_health),
        )
        .route(
            "/api/analytics/realtime/cleanup",
            post(dashboard::realtime_cleanup),
        )
