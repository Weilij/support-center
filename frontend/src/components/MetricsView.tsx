...
function Group({ title, obj }: { title?: string; obj: Record<string, Json> }) {
  ...
  return (
    <div style={{ marginBottom: 14 }}>
      {title && <h4 style={{ margin: '0 0 6px', color: 'var(--ink-2)' }}>{title}</h4>}
      {scalarEntries.length > 0 && (
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {scalarEntries.map(([k, v]) => (
            <div
              key={k}
              style={{ border: '1px solid var(--line)', borderRadius: 8, padding: '8px 12px', minWidth: 120 }}
            >
              <div style={{ fontSize: 12, color: 'var(--muted)' }}>{k}</div>
              <div style={{ fontSize: 18, fontWeight: 600, color: 'var(--ink)' }}>{scalarText(v)}</div>
            </div>
          ))}
        </div>
      )}
      {nestedEntries.map(([k, v]) => (
        <div key={k} style={{ marginTop: 10, paddingLeft: 10, borderLeft: '2px solid var(--line-2)' }}>
          <Group title={k} obj={v as Record<string, Json>} />
        </div>
      ))}
      {arrayEntries.map(([k, v]) => (
        <div key={k} style={{ marginTop: 10 }}>
          <h4 style={{ margin: '0 0 6px', color: 'var(--ink-2)' }}>
            {k}（{(v as Json[]).length}）
          </h4>
